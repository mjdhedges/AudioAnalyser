...
def analyze_channel_low_freq(audio_data: np.ndarray, channel_idx: int, sample_rate: int) -> dict:
    ...
    # Calculate RMS in different frequency bands
    # Use simple filtering to check low frequencies
    from scipy import signal
    
    # 16Hz band (LFE should have strong content here)
    nyquist = sample_rate / 2
    low_16 = 16 / nyquist
    high_16 = 31.25 / nyquist
    sos_16 = signal.butter(4, [low_16, high_16], btype='band', output='sos')
    filtered_16 = signal.sosfiltfilt(sos_16, channel)
    rms_16 = np.sqrt(np.mean(filtered_16**2))
    
    # 31.25Hz band
    low_31 = 31.25 / nyquist
    high_31 = 62.5 / nyquist
    sos_31 = signal.butter(4, [low_31, high_31], btype='band', output='sos')
    filtered_31 = signal.sosfiltfilt(sos_31, channel)
    rms_31 = np.sqrt(np.mean(filtered_31**2))
    
    # 62.5Hz band
    low_62 = 62.5 / nyquist
    high_62 = 125 / nyquist
    sos_62 = signal.butter(4, [low_62, high_62], btype='band', output='sos')
    filtered_62 = signal.sosfiltfilt(sos_62, channel)
    rms_62 = np.sqrt(np.mean(filtered_62**2))
...
def main():
    ...
    print("-" * 80)
    print(f"{'Ch':<4} {'RMS 16Hz':<12} {'RMS 31Hz':<12} {'RMS 62Hz':<12} {'RMS Full':<12} {'Peak':<12} {'LF Ratio':<12}")
    print("-" * 80)
    
    for result in results:
        print(f"{result['channel_idx']:<4} "
