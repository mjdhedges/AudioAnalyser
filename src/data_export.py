"""
Data export module for Audio Analyser.
...
class DataExporter:
    ...
    def calculate_advanced_statistics(
        ...
        clip_events_samples = np.sum(
            audio_dbfs > -0.1
        )  # Actual clipping events (samples)
        peak_saturation = np.sum(
            audio_dbfs > -3.0
        )  # Heavily compressed regions (samples)
        ...
        stats.update(
            {
                ...
                "hot_peaks_rate_per_sec": peaks_above_minus1db
                / duration,  # Use peak count instead
            ...
            stats.update(
                {
                    ...
                    "frequency_crest_factor_range": np.max(octave_crest_factors)
                    - np.min(octave_crest_factors),
                    "bass_vs_treble_dynamics_ratio": np.mean(octave_crest_factors[:3])
                    / max(np.mean(octave_crest_factors[-3:]), 0.1),
                    "frequency_balance_std": np.std(octave_rms_levels),
                    "most_dynamic_frequency_hz": center_freqs[
                        np.argmax(octave_crest_factors)
                    ],
                    "least_dynamic_frequency_hz": center_freqs[
                        np.argmin(octave_crest_factors)
                    ],
                }
            )
        ...
        stats.update(
            {
                ...
                "temporal_crest_factor_range": np.max(time_crest_factors)
                - np.min(time_crest_factors),
            }
        )

        # 4. MASTERING QUALITY INDICATORS
        full_spectrum_stats = analysis_results["statistics"]["Full Spectrum"]
        true_peak_to_rms_ratio = (
            full_spectrum_stats["max_amplitude_db"] - full_spectrum_stats["rms_db"]
        )
        ...
        if total_energy > 0:
            spectral_centroid = (
                sum(
                    freq * energy for freq, energy in zip(center_freqs, octave_energies)
                )
                / total_energy
            )
            bass_energy_ratio = (
                sum(octave_energies[:3]) / total_energy
            )  # 31.25, 62.5, 125 Hz
        else:
        ...
        stats.update(
            {
                ...
                "treble_energy_ratio": sum(octave_energies[-3:])
                / max(total_energy, 1e-10),  # 4k, 8k, 16k Hz
                "mid_energy_ratio": sum(octave_energies[3:7])
                / max(total_energy, 1e-10),  # 250Hz - 2kHz
            }
        )
        ...
        for min_db, max_db, range_name in peak_ranges:
            ...
            stats[f"peak_distribution_{range_name}_percent"] = (
    ...
    def export_comprehensive_results(
        ...
        with open(output_path, "w", newline="") as csvfile:
            csvfile.write("# Audio Analyser - Comprehensive Analysis Results\n")
            ...
            if audio_data is not None:
                ...
                advanced_stats = self.calculate_advanced_statistics(
            ...
            csvfile.write(
                "frequency_hz,max_amplitude,max_amplitude_db,rms,rms_db,dynamic_range,dynamic_range_db,crest_factor,crest_factor_db,mean,std,p10,p25,p50,p75,p90,p95,p99\n"
            )
            ...
            csvfile.write(
                "time_seconds,crest_factor,crest_factor_db,peak_level,rms_level,peak_level_dbfs,rms_level_dbfs\n"
            )
            ...
            valid_crest_db = time_analysis["crest_factors_db"][
                np.isfinite(time_analysis["crest_factors_db"])
            ]
            valid_peak_dbfs = time_analysis["peak_levels_dbfs"][
                np.isfinite(time_analysis["peak_levels_dbfs"])
            ]
            valid_rms_dbfs = time_analysis["rms_levels_dbfs"][
                np.isfinite(time_analysis["rms_levels_dbfs"])
            ]
            ...
            summary_stats = {
                ...
                "time_domain_time_step_seconds": time_analysis.get(
                    "time_step_seconds", np.nan
                ),
                ...
                "crest_factor_mean_db": (
                    np.mean(valid_crest_db) if len(valid_crest_db) > 0 else np.nan
                ),
                "crest_factor_std_db": (
                    np.std(valid_crest_db) if len(valid_crest_db) > 0 else np.nan
                ),
                "crest_factor_min_db": (
                    np.min(valid_crest_db) if len(valid_crest_db) > 0 else np.nan
                ),
                "crest_factor_max_db": (
                    np.max(valid_crest_db) if len(valid_crest_db) > 0 else np.nan
                ),
                "peak_level_mean_dbfs": (
                    np.mean(valid_peak_dbfs) if len(valid_peak_dbfs) > 0 else np.nan
                ),
                "peak_level_std_dbfs": (
                    np.std(valid_peak_dbfs) if len(valid_peak_dbfs) > 0 else np.nan
                ),
                "rms_level_mean_dbfs": (
                    np.mean(valid_rms_dbfs) if len(valid_rms_dbfs) > 0 else np.nan
                ),
                "rms_level_std_dbfs": (
                    np.std(valid_rms_dbfs) if len(valid_rms_dbfs) > 0 else np.nan
                ),
            }
            ...
            if audio_data is not None:
                ...
                for freq in center_freqs:
                    ...
                    if freq_key in statistics:
                        ...
                        # Remove DC component, tiny values, and samples outside
                        # the exported histogram range.
                        clean_signal = signal[
                            (np.abs(signal) > 1e-10)
                            & (signal >= -1.0)
                            & (signal <= 1.0)
                        ...
                        if len(clean_signal) > 0:
                            ...
                            hist, bin_edges = np.histogram(
                                clean_signal, bins=51, range=(-1, 1), density=True
                            )
                            bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

                            for bin_center, count, density in zip(
                                bin_centers, hist, hist
                            ):
                                csvfile.write(
                                    f"{freq},{bin_center},{count},{density}\n"
                                )
            ...
            if chunk_octave_analysis is not None:
                ...
                csvfile.write(
                    "chunk_type,time_seconds,crest_factor_db,frequency_hz,max_amplitude_db,rms_db,chunk_crest_factor_db\n"
                )
                ...
                if min_chunk_data is not None:
                    ...
                    for freq in center_freqs:
                        ...
                        if freq_key in min_analysis["statistics"]:
                            ...
                            csvfile.write(
                                f"min_crest,{min_time},{min_crest_db},{freq},"
                ...
                if max_chunk_data is not None:
                    ...
                    for freq in center_freqs:
                        ...
                        if freq_key in max_analysis["statistics"]:
                            ...
                            csvfile.write(
                                f"max_crest,{max_time},{max_crest_db},{freq},"
                                f"{stats['max_amplitude_db']},{stats['rms_db']},{stats['crest_factor_db']}\n"
                            )
            ...
            if envelope_statistics is not None:
                csvfile.write("\n[ENVELOPE_STATISTICS]\n")
                csvfile.write(
                    "frequency_hz,analysis_type,rank,peak_value_db,peak_time_seconds,attack_time_ms,peak_hold_time_ms,decay_3db_ms,decay_6db_ms,decay_9db_ms,decay_12db_ms,decay_12db_reached\n"
                )
                ...
                csvfile.write(
                    "frequency_hz,pattern_num,num_repetitions,mean_interval_seconds,std_interval_seconds,median_interval_seconds,min_interval_seconds,max_interval_seconds,pattern_regularity_score,pattern_confidence,beats_per_minute\n"
                )
                ...
                has_events = any(
                    "sustained_peaks_events" in band
                    for band in envelope_statistics.values()
                )
                if has_events:
                    csvfile.write("\n[SUSTAINED_PEAKS_EVENTS]\n")
                    csvfile.write(
                        "frequency_hz,peak_time_seconds,peak_value_db,hold_ms,t3_ms,t6_ms,t9_ms,t12_ms\n"
                    )
                    for freq_label, band_data in envelope_statistics.items():
