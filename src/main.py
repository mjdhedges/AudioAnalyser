"""
Main entry point for the Audio Analyser application.
...
from src.post.lfe_octave_time import (
    generate_lfe_octave_time_plot,
...
def get_config_hash() -> str:
    ...
    relevant_config = {
        ...
        "octave_fft_block_duration_seconds": config.get(
            "analysis.octave_fft_block_duration_seconds", 30.0
        ),
        "octave_max_memory_gb": config.get("analysis.octave_max_memory_gb", 4.0),
        "octave_include_low_residual_band": config.get(
            "analysis.octave_include_low_residual_band", True
        ),
        "octave_include_high_residual_band": config.get(
            "analysis.octave_include_high_residual_band", True
        ),
        "octave_low_residual_center_hz": config.get(
            "analysis.octave_low_residual_center_hz", 4.0
        ),
        "octave_high_residual_center_hz": config.get(
            "analysis.octave_high_residual_center_hz", 32000.0
        ),
        "sustained_peaks_search_window_seconds": config.get(
...
def save_result_cache(
    track_path: Path, track_output_dir: Path, config_hash: str
) -> None:
...
def analyze_single_track(
    ...
    try:
        ...
        enable_result_cache = use_cache and config.get(
            "performance.enable_result_cache", True
        )
        ...
        audio_data, sr = audio_processor.load_audio(
            track_path, start_time=test_start_time, duration=test_duration
        )
        ...
        logger.info(
            f"Audio info: {channel_layout}, {num_channels} channel(s), "
        ...
        if filter_specs:
            unmatched = {
                spec["raw"]
                for spec in filter_specs
                if spec["raw"] not in matched_filters
            }
            if unmatched:
                logger.warning(
...
@click.option("--dpi", "-d", type=int, help="DPI for plot output (overrides config)")
@click.option(
    "--log-level",
    "-l",
...
@click.option(
    "--export-csv/--no-export-csv", default=True, help="Export results to CSV"
)
@click.option(
...
def main(
    ...
    generate_decay_plot: bool,
    run_group_plots: bool,
    run_lfe_deep_dive: bool,
    run_screen_deep_dive: bool,
    run_surround_deep_dive: bool,
    post_only: bool,
    test_start_time: Optional[float],
    ...
    try:
        ...
        if skip_octave_cf_time is None:
            skip_octave_cf_time = bool(
                config.get("performance.skip_octave_cf_time", False)
            )
        if export_octave_cf_time_data is None:
            export_octave_cf_time_data = bool(
                config.get("performance.export_octave_cf_time_data", False)
            )
        ...
        def _run_select_worst_channels(track_dir: Path) -> bool:
            try:
                ...
                logger.warning(
                    f"Failed to generate worst-channel manifest for {track_dir.name}: {e}"
                )
                return False
        ...
        def _run_group_decay_plot(track_dir: Path) -> bool:
            try:
                ...
                logger.warning(
                    f"Failed to generate decay plot for {track_dir.name}: {e}"
                )
                return False
        ...
        def _run_group_crest_factor_time_plot(track_dir: Path) -> bool:
            try:
                ...
                logger.info(
                    f"Group crest factor time plots generated for {track_dir.name}"
                )
                return True
            except Exception as e:
                logger.warning(
                    f"Failed to generate group crest factor time plots for {track_dir.name}: {e}"
                )
                return False
        ...
        def _run_group_octave_spectrum_plot(track_dir: Path) -> bool:
            try:
                ...
                logger.info(
                    f"Group octave spectrum plots generated for {track_dir.name}"
                )
                return True
            except Exception as e:
                logger.warning(
                    f"Failed to generate group octave spectrum plots for {track_dir.name}: {e}"
                )
                return False
        ...
        def _run_lfe_full_channel_plot(track_dir: Path) -> bool:
            try:
                ...
                logger.warning(
                    f"Failed to generate LFE full channel plot for {track_dir.name}: {e}"
                )
                return False

        def _run_lfe_octave_time_plot(
            track_dir: Path, original_track_path: Optional[Path] = None
        ) -> bool:
            try:
                output_path = track_dir / "LFE" / "lfe_octave_time.png"
                result = generate_lfe_octave_time_plot(
                    track_dir, output_path, original_track_path
                )
                if result:
                    logger.info(
                        f"LFE octave band time plot generated for {track_dir.name}"
                    )
                    return True
                return False
            except Exception as e:
                logger.warning(
                    f"Failed to generate LFE octave band time plot for {track_dir.name}: {e}"
                )
                return False

        def _run_screen_deep_dive_plot(
            track_dir: Path, original_track_path: Optional[Path] = None
        ) -> bool:
            try:
                output_dir = track_dir / "Screen"
                result = generate_channel_deep_dive_plot(
                    track_dir, "Screen", output_dir, original_track_path
                )
                if result:
                    logger.info(
                        f"Screen deep dive plots generated for {track_dir.name}"
                    )
                    return True
                return False
            except Exception as e:
                logger.warning(
                    f"Failed to generate Screen deep dive plots for {track_dir.name}: {e}"
                )
                return False

        def _run_surround_deep_dive_plot(
            track_dir: Path, original_track_path: Optional[Path] = None
        ) -> bool:
            try:
                output_dir = track_dir / "Surround+Height"
                result = generate_channel_deep_dive_plot(
                    track_dir, "Surround+Height", output_dir, original_track_path
                )
                if result:
                    logger.info(
                        f"Surround+Height deep dive plots generated for {track_dir.name}"
                    )
                    return True
                return False
            except Exception as e:
                logger.warning(
                    f"Failed to generate Surround+Height deep dive plots for {track_dir.name}: {e}"
                )
                return False
        ...
        if batch:
            ...
            enable_parallel_batch = config.get(
                ...
                if not skip_post:
                    ...
                    if track_dir.exists():
                        ...
                        if run_lfe_deep_dive:
                            ...
                            _run_lfe_octave_time_plot(
                                track_dir, original_track_path=input
                            )
                        if run_screen_deep_dive:
                            _run_screen_deep_dive_plot(
                                track_dir, original_track_path=input
                            )
                        if run_surround_deep_dive:
                            _run_surround_deep_dive_plot(
                                track_dir, original_track_path=input
                            )
                    else:
                        logger.warning(
