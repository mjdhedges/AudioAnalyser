"""
Visualization module for Audio Analyser.
...
# Default log-axis ticks for octave-band spectrum / crest plots (Hz -> label)
...
def _sorted_octave_band_rows(
    statistics: Dict[str, Dict],
) -> list[tuple[float, str, Dict]]:
    """Return (center_hz, statistics_key, stats_dict) sorted by ascending center."""
    rows: list[tuple[float, str, Dict]] = []
    for key, st in statistics.items():
        if key == "Full Spectrum":
            continue
        try:
            fc = float(key)
        except (TypeError, ValueError):
            continue
        rows.append((fc, key, st))
    rows.sort(key=lambda x: x[0])
    return rows


def _octave_semilog_ticks(plot_freqs: np.ndarray) -> tuple[list[float], list[str]]:
    """X tick positions and labels for octave-band semilog plots."""
    if plot_freqs.size == 0:
        return [f for f, _ in _DEFAULT_OCTAVE_TICKS_HZ], [
            lb for _, lb in _DEFAULT_OCTAVE_TICKS_HZ
        ]
    min_f = float(np.min(plot_freqs[np.isfinite(plot_freqs)]))
    ticks: list[float] = []
    labels: list[str] = []
    for f_hz, lab in _DEFAULT_OCTAVE_TICKS_HZ:
        if f_hz >= min_f * 0.85:
            ticks.append(f_hz)
            labels.append(lab)
    for p in plot_freqs:
        if not np.isfinite(p):
            continue
        pf = float(p)
        if not ticks or min(abs(pf - t) / max(t, 1e-9) for t in ticks) > 0.04:
            ticks.append(pf)
            labels.append(
                f"{pf:g}" if pf >= 100 else f"{pf:.3g}".rstrip("0").rstrip(".")
            )
...
class PlotGenerator:
    ...
    def create_octave_spectrum_plot(
        ...
        band_rows = _sorted_octave_band_rows(statistics)
        if not band_rows:
            logger.warning("No octave band statistics to plot for octave spectrum")
            plt.close(fig)
            return

        plot_freqs = np.array([fc for fc, _, _ in band_rows], dtype=float)
        max_db_values = []
        rms_db_values = []
        for _fc, key, st in band_rows:
            max_db = st["max_amplitude_db"]
            rms_db = st["rms_db"]
        ...
        plot_max_db = np.asarray(max_db_values, dtype=float)
        plot_rms_db = np.asarray(rms_db_values, dtype=float)

        # Replace -inf with a very low value for plotting
        plot_max_db[plot_max_db == -np.inf] = -60.0
        plot_rms_db[plot_rms_db == -np.inf] = -60.0

        ax.semilogx(
            plot_freqs, plot_max_db, "b-o", label="Max Peak (dBFS)", linewidth=2
        )
        ...
        if time_analysis is not None and chunk_octave_analysis is not None:
            self._add_extreme_chunk_analysis(
                ax, time_analysis, chunk_octave_analysis, plot_freqs.tolist()
            )
        ...
        ax.set_title(
            self._format_title(
                "Octave Band Analysis - Peak and RMS Levels (dBFS)",
                track_name,
                channel_name,
            )
        )
        ...
        ax.legend()
        xlim = app_config.get("plotting.octave_spectrum_xlim", [7, 20000])
        try:
        ...
        x_left_data = (
            float(np.min(plot_freqs)) / 1.25 if plot_freqs.size else x_left_cfg
        )
        ax.set_xlim([min(x_left_cfg, x_left_data), x_right])
        ax.set_ylim([-60, 3])

        tick_x, tick_lbl = _octave_semilog_ticks(plot_freqs)
    ...
    def _add_extreme_chunk_analysis(
        ...
        if min_chunk_data is not None:
            ...
            for (
                freq
            ) in center_freqs:  # center_freqs now only contains actual frequency bands
                freq_key = f"{freq:.3f}"
                if freq_key in min_analysis["statistics"]:
                    min_rms_db.append(min_analysis["statistics"][freq_key]["rms_db"])
                    min_peak_db.append(
                        min_analysis["statistics"][freq_key]["max_amplitude_db"]
                    )
                else:
            ...
            full_spectrum_peak = min_analysis["statistics"]["Full Spectrum"][
                "max_amplitude_db"
            ]
        ...
        if max_chunk_data is not None:
            ...
            for (
                ...
                if freq_key in max_analysis["statistics"]:
                    ...
                    max_peak_db.append(
                        max_analysis["statistics"][freq_key]["max_amplitude_db"]
                    )
                else:
            ...
            # Get full spectrum peak for legend
            full_spectrum_peak = max_analysis["statistics"]["Full Spectrum"][
                "max_amplitude_db"
            ]
    ...
    def create_histogram_plots(
        ...
        if band_data is None and octave_bank is not None:
            ...
            extended_freqs = (
                [0] + center_freqs if center_freqs[0] != 0 else center_freqs
            )
        ...
        # Create enough subplots for all configured bands, including residual bands.
        num_cols = 4
            ...
            if len(clean_signal) == 0:
                ...
                try:
                    in_range = clean_signal[
                        (clean_signal >= -1.0) & (clean_signal <= 1.0)
                    ]
                    if in_range.size == 0:
                        raise ValueError("no samples in histogram range")
                    ax.hist(in_range, bins=51, alpha=0.7, density=True, range=(-1, 1))
        ...
        plt.suptitle(
            self._format_title(
                "Amplitude Distribution by Octave Band", track_name, channel_name
            ),
            fontsize=16,
        )
        plt.tight_layout()
    ...
    def create_histogram_plots_log_db(
        ...
        noise_floor_db = (
            config.get("log_histogram_noise_floor_db", -60) if config else -60
        )
        max_db = config.get("log_histogram_max_db", 0) if config else 0
        max_bin_size_db = (
            config.get("log_histogram_max_bin_size_db", 3.0) if config else 3.0
        )
        ...
        logger.info(
            f"Log histogram range: {noise_floor_db} to {max_db} dBFS with {min_bins} bins (max bin size: {max_bin_size_db} dB)"
        )
        ...
        for i, (freq_label, signal) in enumerate(band_data.items()):
            ...
            if len(clean_signal) == 0:
                ...
                try:
                    ...
                    signal_db = signal_db[
                        (signal_db >= noise_floor_db) & (signal_db <= max_db)
                    ]
                    if signal_db.size == 0:
                        raise ValueError("no samples in log histogram range")
                    ...
                    logger.warning(
                        f"Could not create log dB histogram for {freq_label}: {e}"
                    )
                    ax.text(
                        0.5,
                        0.5,
        ...
        plt.suptitle(
            self._format_title(
                "Amplitude Distribution by Octave Band (Log dBFS Scale)",
                track_name,
                channel_name,
            ),
            fontsize=16,
    ...
    def create_crest_factor_time_plot(
        ...
        peak_levels_linear = np.asarray(
            time_analysis.get("peak_levels", []), dtype=np.float64
        )
        rms_levels_linear = np.asarray(
            time_analysis.get("rms_levels", []), dtype=np.float64
        )
        ...
        channel_label = (
            (channel_name or "").replace("Channel ", "").replace("Channel", "").strip()
        )
        ...
        ax1.set_title(
            self._format_title("Crest Factor vs Time", track_name, channel_name)
        )
        ...
        ax2.set_title(
            self._format_title("Peak and RMS Levels vs Time", track_name, channel_name)
        )
    ...
    def create_octave_crest_factor_time_plot(
        ...
        ax.set_title(
            self._format_title(
                "Octave Band Crest Factor vs Time", track_name, channel_name
            )
        )
        ax.grid(True, alpha=0.3)
    ...
    def export_octave_crest_factor_time_csv(
        ...
        for freq in center_frequencies:
            ...
            out[key] = np.asarray(
                octave_crest_factors.get(freq, [0.0] * len(time_points)),
                dtype=np.float64,
            )
        df = pd.DataFrame(out)
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(output_path, index=False)
        logger.info(f"Octave crest factor time data saved to: {output_path}")
    ...
    def create_pattern_envelope_plots(
        ...
        for band_idx, freq in enumerate(extended_frequencies):
            ...
            for pattern_num in range(1, patterns_detected + 1):
                ...
                for idx, (peak_idx, peak_time) in enumerate(
                    zip(peak_indices, peak_times)
                ):
                    ...
                    if (
                        stored_windows is not None
                        and idx < len(stored_windows)
                        and stored_windows[idx] is not None
                    ):
                        ...
                        time_relative_ms = (
                            time_window - time_window[peak_time_idx]
                        ) * 1000.0
            ...
            if pattern_dir:
                output_path = (
                    pattern_dir
                    / f"pattern_envelopes_{freq_label.replace(' ', '_').replace('.', '_')}.png"
                )
                plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
    ...
    def create_independent_envelope_plots(
        ...
        for band_idx, freq in enumerate(extended_frequencies):
            ...
            top_envelopes = worst_case_envelopes[
                : min(num_envelopes, len(worst_case_envelopes))
            ]
            ...
            for idx, envelope_data in enumerate(top_envelopes):
                ...
                if (
                    "envelope_window" in envelope_data
                    and envelope_data["envelope_window"] is not None
                ):
                ...
                label = (
                    f"Rank {envelope_data['rank']} - Peak: {peak_value_db:.1f} dBFS @ "
                ...
                for decay_db in [-3, -6, -9, -12]:
                    ...
                    if decay_key in decay_times:
                        ...
                        decay_reached = decay_times.get(
                            f"decay_{abs(decay_db)}db_reached", False
                        )
            ...
            # Check if window was capped and add note to title
            base_title = (
                f"Top {len(top_envelopes)} Independent Envelopes - {freq_label}"
            )
            if window_was_capped and freq > 0:
                expected_window_ms = (num_wavelengths / freq) * 1000.0
                base_title += f"\n(Window capped at 500ms, {num_wavelengths}λ would be {expected_window_ms:.0f}ms)"
            ...
            if independent_dir:
                output_path = (
                    independent_dir
                    / f"independent_envelopes_{freq_label.replace(' ', '_').replace('.', '_')}.png"
                )
                plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
    ...
    def create_crest_factor_plot(
        ...
        band_rows = _sorted_octave_band_rows(statistics)
        if not band_rows:
            logger.warning("No octave band statistics to plot for crest factor plot")
            plt.close(fig)
            return
        ...
        for _fc, key, st in band_rows:
            crest_factor_db = st["crest_factor_db"]
            crest_vals.append(
                crest_factor_db if np.isfinite(crest_factor_db) else -np.inf
            )

        plot_crest_db = np.asarray(crest_vals, dtype=float)
        plot_crest_db[plot_crest_db == -np.inf] = 0.0
        plot_crest_db = np.maximum(plot_crest_db, 0.0)
        ...
        if chunk_octave_analysis is not None:
            ...
            if min_chunk_data is not None:
                ...
                min_octave_crest_db = []
                for freq in plot_freqs:  # Hz values for each octave band column
                    freq_key = f"{freq:.3f}"
                    if freq_key in min_analysis["statistics"]:
                        crest_db = min_analysis["statistics"][freq_key].get(
                            "crest_factor_db", 0.0
                        )
                        min_octave_crest_db.append(
                            crest_db if np.isfinite(crest_db) else 0.0
                        )
                    else:
            ...
            if max_chunk_data is not None:
                ...
                for freq in plot_freqs:  # Hz values for each octave band column
                    freq_key = f"{freq:.3f}"
                    if freq_key in max_analysis["statistics"]:
                        crest_db = max_analysis["statistics"][freq_key].get(
                            "crest_factor_db", 0.0
                        )
                        max_octave_crest_db.append(
                            crest_db if np.isfinite(crest_db) else 0.0
                        )
                    else:
        ...
        ax.set_ylabel("Crest Factor (dB)")
        ax.set_title(
            self._format_title("Crest Factor by Octave Band", track_name, channel_name)
        )
        ax.grid(True, alpha=0.3)
        ax.legend()
        xlim = app_config.get("plotting.crest_factor_xlim", [7, 20000])
        try:
            x_left_cfg = float(xlim[0])
            x_right = float(xlim[1])
        except (TypeError, ValueError, IndexError):
