...
def generate_group_crest_factor_time_plot(track_dir: Path, output_dir: Path) -> Dict[str, Path]:
    ...
    for group_name, channels in group_channels.items():
        ...
        for idx, (channel_folder, csv_path) in enumerate(channels):
            ...
            valid_cf = crest_factor_db_plot[np.isfinite(crest_factor_db_plot) & (crest_factor_db_plot > -60)]
            if len(valid_cf) > 0:
