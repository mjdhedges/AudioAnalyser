...
def generate_group_octave_spectrum_plot(track_dir: Path, output_dir: Path) -> Dict[str, Path]:
    ...
    for group_name, channels in group_channels.items():
        ...
        for idx, (channel_folder, csv_path) in enumerate(channels):
            ...
            crest_factor_db_plot[crest_factor_db_plot == -np.inf] = 0.0
            ...
            channel_label = channel_folder.replace("Channel ", "").replace("Channel", "").strip()
            ...
            if octave_data.get("track_peak_db") is not None and octave_data.get("track_rms_db") is not None:
