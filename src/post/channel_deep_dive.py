...
def _find_channel_folders(track_dir: Path, group_name: str) -> List[Tuple[str, Path]]:
    ...
    for sub in track_dir.iterdir():
        ...
        elif group_name == "Surround+Height" and any(folder.startswith(prefix) for prefix in surround_prefixes):
...
def _get_channel_index_from_folder(folder_name: str, channel_layout: str) -> Optional[int]:
...
def generate_channel_deep_dive_plot(track_dir: Path, group_name: str, output_dir: Path, 
    ...
    for channel_folder, csv_path in channels:
        ...
        if len(channel_data) < 1000:
            logger.warning(f"Channel {channel_folder} too short for octave bank processing: {len(channel_data)} samples")
            continue
    ...
    for band_idx, center_freq in enumerate(octave_bands):
        freq_label = f"{center_freq:.1f} Hz" if center_freq < 100 else f"{center_freq:.0f} Hz"
        ...
        for idx, (channel_label, (ch_time, ch_crest)) in enumerate(channel_crest_data.items()):
        ...
        for idx, (channel_label, (ch_time, ch_peak)) in enumerate(channel_peak_data.items()):
            color = channel_colors[idx % len(channel_colors)]
            ax2.plot(
                ch_time,
                ch_peak,
        ...
        for idx, (channel_label, (ch_time, ch_rms)) in enumerate(channel_rms_data.items()):
            color = channel_colors[idx % len(channel_colors)]
            ax2.plot(
                ch_time,
                ch_rms,
        ...
        ax2.set_title(f"{group_name} {freq_label} Octave Band Peak and RMS Levels Over Time - {track_name}")
        ...
        freq_filename = f"{group_name.lower().replace('+', '_')}_{freq_str_filename}Hz.png"
        ...
        logger.info(f"Generated {group_name} {freq_label} octave band time plot (all channels): {freq_output_path}")
