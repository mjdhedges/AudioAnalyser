...
class TestMKVSupport:
    ...
    def test_extract_truehd_from_mkv(self, mock_tempfile, mock_subprocess):
        ...
        mkv_path = Path("test.mkv")
        temp_wav_path = self.processor._extract_truehd_from_mkv(mkv_path, stream_index=1)
    ...
    @patch('src.audio_processor.AudioProcessor._extract_truehd_from_mkv')
    @patch('pathlib.Path.exists')
    def test_load_audio_mkv_no_truehd_stream(self, mock_exists, mock_extract, mock_probe):
    ...
    def test_mkv_support_disabled(self):
        ...
        processor_no_mkv = AudioProcessor(
            sample_rate=44100,
            enable_mkv_support=False
        )
